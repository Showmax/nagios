...
RE_DEFINE_END = re.compile("^[\s\t]*}[\s\t]*$")
RE_ICFLIX = re.compile("icflix\.(com|io)$")
...
class NagiosToYaml(object):
    ...
    def fix_hostname(self, obj):
        ...
        while i < len(hosts):
            if not RE_ICFLIX.search(hosts[i]):
                logging.debug("Fixing %s to %s", hosts[i],
                        "%s.icflix.com" % (hosts[i]))
    ...
    def process_object(self, obj):
        ...
        if stor_key not in self.nagios_cfg.keys():
        ...
        if obj_type == "service":
            ...
            if "passive_checks_enabled" in obj.keys():
    ...
    def process_hostgroups(self):
        ...
        if "hostgroups" not in self.nagios_cfg.keys():
            return

        hostgroups = self.nagios_cfg["hostgroups"]
        for hostgroup in hostgroups.keys():
            ...
            for member in hostgroups[hostgroup]["members"].split(","):
                ...
                if fqdn not in self.nagios_cfg["hosts"].keys():
                    logging.warning("%s(former: %s) not found.", fqdn,
                            repr(member))
                    members.append(member)
                    continue

                if "hostgroups" not in self.nagios_cfg["hosts"][fqdn].keys():
